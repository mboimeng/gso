...
function find () {
	...
	var da = fs.readFileSync(path.join(__dirname, 'ips'), {encoding: 'utf8'});
	da = da.split('\n');
	console.log(da);
	return da;
}
		...
		(function (ip) {
		    ...
		    request(options, function (err, res, body) {
				...
				if (search.length >= 7 && useTime <= timeout) {
					fs.writeFileSync(path.join(__dirname, 'good_ip'), ip+"\n", {encoding: 'utf8', flag: 'a'});
				}
		        }
		        
			if (++index === total) {
				var da = fs.readFileSync(path.join(__dirname, 'good_ip'), {encoding: 'utf8'});
				da = da.split('\n');
				da.pop();
				console.log(da);
			}
...
if (fs.existsSync(path.join(__dirname, 'good_ip'))) {
	fs.unlinkSync(path.join(__dirname, 'good_ip'));
}
check();