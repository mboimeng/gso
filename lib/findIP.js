...
var good_ipPath = path.join(__dirname, 'good_ip');
var goodIPs = [];
		...
		(function (ip) {
		    ...
		    request(options, function (err, res, body) {
				...
				if (search.length >= 7 && useTime <= timeout) {
					fs.writeFileSync(good_ipPath, ip+"\n", {encoding: 'utf8', flag: 'a'});
				}
		        }
		        
			if (++index === total) {
				var da = fs.readFileSync(good_ipPath, {encoding: 'utf8'});
				da = da.split('\n');
				da.pop();
				console.log(da);
			}
...
if (fs.existsSync(good_ipPath)) {
	fs.unlinkSync(good_ipPath);
}
check();